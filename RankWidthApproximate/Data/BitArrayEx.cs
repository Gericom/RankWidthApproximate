...
{
    // Modified version of the build-in BitArray class

    // A vector of bits.  Use this to store bits efficiently, without having to do bit
    // shifting yourself.
    [Serializable]
    [TypeForwardedFrom("mscorlib, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089")]
    public sealed class BitArrayEx : ICollection, ICloneable, IEquatable<BitArrayEx>
    {
        ...
        ** Exceptions: ArgumentException if bytes == null.
        ...
        public unsafe BitArrayEx(bool[] values)
        {
            if (values == null)
            {
        ...
        ** Exceptions: ArgumentException if values == null.
        ...
        /*=========================================================================
        ** Allocates a new BitArrayEx with the same length and bit values as bits.
        **
        ** Exceptions: ArgumentException if bits == null.
        ...
        public BitArrayEx(BitArrayEx bits)
        {
            ...
            // m_array = (int[])bits.m_array.Clone();
            m_array = GC.AllocateUninitializedArray<int>(bits.m_array.Length);
            Buffer.BlockCopy(bits.m_array, 0, m_array, 0, bits.m_array.Length * sizeof(int));
            // Unsafe.CopyBlock();
        ...
        **             index >= GetLength().
        ...
        public bool Get(int index)
        {
            ...
            ref var data = ref MemoryMarshal.GetArrayDataReference(m_array);
            return (Unsafe.Add(ref data, index >> 5) & (1 << index)) != 0;
        ...
        ** Sets all the bit values to value.
        ...
        public void SetAll(bool value)
        {
            ...
            if (value)
            {
                ...
                if ((Length & 0x1F) != 0)
                {
                    uint lastMask = 0xFFFFFFFFu << (Length & 0x1F);
                    m_array[^1] &= (int)~lastMask;
                }
            }
            else
                span.Clear();
        ...
        **             value.Length != this.Length.
        =========================================================================*/
        public unsafe BitArrayEx And(BitArrayEx value)
        {
            if (value is null)
            ...
            // corrupting the BitArrayEx data in a way that produces incorrect answers, since BitArrayEx is not meant
        ...
        public unsafe BitArrayEx AndNot(BitArrayEx value)
        {
            if (value is null)
                throw new ArgumentNullException(nameof(value));
            ...
            if (Avx2.IsSupported)
            {
                ...
                fixed (int* rightPtr = valueArray)
                {
                    for (; i < (uint)count - (Vector256IntCount - 1u); i += Vector256IntCount)
                    {
                        Vector256<int> leftVec  = Avx.LoadVector256(leftPtr + i);
                        Vector256<int> rightVec = Avx.LoadVector256(rightPtr + i);
                        Avx.Store(leftPtr + i, Avx2.AndNot(leftVec, rightVec));
                    }
                }
            }
            else if (Sse2.IsSupported)
            {
                ...
                fixed (int* rightPtr = valueArray)
                {
                    for (; i < (uint)count - (Vector128IntCount - 1u); i += Vector128IntCount)
                    {
                        Vector128<int> leftVec  = Sse2.LoadVector128(leftPtr + i);
                        Vector128<int> rightVec = Sse2.LoadVector128(rightPtr + i);
                        Sse2.Store(leftPtr + i, Sse2.AndNot(leftVec, rightVec));
                    }
                }
            }
            else if (AdvSimd.IsSupported)
            {
                ...
                fixed (int* rightPtr = valueArray)
                {
                    for (; i < (uint)count - (Vector128IntCount - 1u); i += Vector128IntCount)
                    {
                        Vector128<int> leftVec  = AdvSimd.LoadVector128(leftPtr + i);
                        Vector128<int> rightVec = AdvSimd.LoadVector128(rightPtr + i);
                        AdvSimd.Store(leftPtr + i, AdvSimd.BitwiseClear(leftVec, rightVec));
                    }
                }
            }
        ...
        **             value.Length != this.Length.
        =========================================================================*/
        public unsafe BitArrayEx Or(BitArrayEx value)
        {
            if (value is null)
                throw new ArgumentNullException(nameof(value));
        ...
        **             value.Length != this.Length.
        =========================================================================*/
        public unsafe BitArrayEx Xor(BitArrayEx value, int start = 0)
        {
            if (value is null)
                throw new ArgumentNullException(nameof(value));
            ...
            if (Avx2.IsSupported)
            {
                i = (uint)((start >> 8) * Vector256IntCount);
                fixed (int* leftPtr = thisArray)
                fixed (int* rightPtr = valueArray)
                {
                    for (; i < (uint)count - (Vector256IntCount - 1u); i += Vector256IntCount)
            ...
            {
                i = (uint)((start >> 7) * Vector128IntCount);
                fixed (int* leftPtr = thisArray)
                fixed (int* rightPtr = valueArray)
                {
                    for (; i < (uint)count - (Vector128IntCount - 1u); i += Vector128IntCount)
        ...
        public static unsafe BitArrayEx Xor(BitArrayEx a, BitArrayEx b)
        {
            Debug.Assert(a is not null);
            Debug.Assert(b is not null);
            Debug.Assert(a.Length == b.Length);

            var dst = new BitArrayEx();
            dst.m_array  = GC.AllocateUninitializedArray<int>(a.m_array.Length);
            dst.m_length = a.m_length;

            fixed (int* aPtr = a.m_array)
            fixed (int* bPtr = b.m_array)
            fixed (int* dstPtr = dst.m_array)
            {
                int count = GetInt32ArrayLengthFromBitLength(dst.Length);
                switch (count)
                {
                    case 7:
                ...
                if (Avx2.IsSupported)
                {
                    ...
                    for (; i < (uint)count - (Vector256IntCount - 1u); i += Vector256IntCount)
                    {
                        var leftVec  = Avx.LoadVector256(aPtr + i);
                        var rightVec = Avx.LoadVector256(bPtr + i);
                        Avx.Store(dstPtr + i, Avx2.Xor(leftVec, rightVec));
                    }
                }
                else if (Sse2.IsSupported)
                {
                    ...
                    for (; i < (uint)count - (Vector128IntCount - 1u); i += Vector128IntCount)
                    {
                        var leftVec  = Sse2.LoadVector128(aPtr + i);
                        var rightVec = Sse2.LoadVector128(bPtr + i);
                        Sse2.Store(dstPtr + i, Sse2.Xor(leftVec, rightVec));
                    }
                }
                else if (AdvSimd.IsSupported)
                {
                    ...
                    for (; i < (uint)count - (Vector128IntCount - 1u); i += Vector128IntCount)
                    {
                        var leftVec  = AdvSimd.LoadVector128(aPtr + i);
                        var rightVec = AdvSimd.LoadVector128(bPtr + i);
                        AdvSimd.Store(dstPtr + i, AdvSimd.Xor(leftVec, rightVec));
                    }
                }

                for (; i < (uint)count; i++)
                    dstPtr[i] = aPtr[i] ^ bPtr[i];
            }
        ...
        ** is updated and returned.
        =========================================================================*/
        public unsafe BitArrayEx Not()
        {
            // This method uses unsafe code to manipulate data in the BitArrayEx.  To avoid issues with
        ...
        ** Exceptions: ArgumentOutOfRangeException if count < 0
        ...
        public unsafe void CopyTo(Array array, int index)
        {
            ...
            else if (array is bool[] boolArray)
            {
                ...
                if (Avx2.IsSupported)
                {
                    Vector256<byte> shuffleMask = Vector256.Create(s_lowerShuffleMask_CopyToBoolArray,
        ...
        public object Clone() => new BitArrayEx(this);

        public void Set(BitArrayEx src)
        {
            Debug.Assert(Length == src.Length);
            // Array.Copy(src.m_array, m_array, m_array.Length);
            Buffer.BlockCopy(src.m_array, 0, m_array, 0, src.m_array.Length * sizeof(int));
            _version++;
        }

        public unsafe bool Equals(BitArrayEx other)
        {
            ...
            switch (m_array.Length)
            {
                case 8:
                    if (m_array[7] != other.m_array[7])
                        return false;
                    goto case 7;
                case 7:
                    if (m_array[6] != other.m_array[6])
                        return false;
                    goto case 6;
                case 6:
                    if (m_array[5] != other.m_array[5])
                        return false;
                    goto case 5;
                case 5:
                    if (m_array[4] != other.m_array[4])
                        return false;
                    goto case 4;
                case 4:
                    if (m_array[3] != other.m_array[3])
                        return false;
                    goto case 3;
                case 3:
                    if (m_array[2] != other.m_array[2])
                        return false;
                    goto case 2;
                case 2:
                    if (m_array[1] != other.m_array[1])
                        return false;
                    goto case 1;
                case 1:
            ...
            if (Avx2.IsSupported)
            {
                fixed (int* aData = &m_array[0])
                fixed (int* bData = &other.m_array[0])
                {
                    for (; i < m_array.Length - 7; i += 8)
                    {
                        var aVec = Avx.LoadVector256(aData + i);
                        var bVec = Avx.LoadVector256(bData + i);
                        if (!Avx.TestZ(Avx2.Xor(aVec, bVec), Vector256<int>.AllBitsSet))
                            return false;
                    }
                }
            }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj.GetType() != this.GetType())
                return false;
            return Equals((BitArrayEx)obj);
        }

        public override int GetHashCode()
        {
            int hc = m_array.Length;
            for (int i = 0; i < m_array.Length; i++)
                hc = unchecked(hc * 314159 + m_array[i]);

            return hc;
        }

        public static bool operator ==(BitArrayEx left, BitArrayEx right) => Equals(left, right);

        public static bool operator !=(BitArrayEx left, BitArrayEx right) => !Equals(left, right);

        public IEnumerator GetEnumerator() => new BitArrayEnumeratorSimple(this);

        public int FindNextOne(int start, int end = -1)
        {
            ...
            int val = m_array[i];
            val &= ~((1 << (start & 0x1F)) - 1);
            int ctz = BitOperations.TrailingZeroCount(val);
            if (ctz != 32)
            {
                int result = (i << 5) + ctz;
                if (end > 0 && result >= end)
            ...
            int endI = end > 0 ? Math.Min((end + 31) >> 5, m_array.Length) : m_array.Length;

            for (; i < endI; i++)
            {
                ctz = BitOperations.TrailingZeroCount(m_array[i]);
                if (ctz != 32)
                {
                    int result = (i << 5) + ctz;
                    if (end > 0 && result >= end)
        ...
        {
            private readonly BitArrayEx _bitArray;
            private          int        _index;
            ...
            {
                _bitArray = BitArrayEx;
                _index    = -1;
                _version  = BitArrayEx._version;
            }
